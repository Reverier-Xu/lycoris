//! Plugin manifest model and wire-format (de)serialization.
//!
...
pub struct PluginManifest {
  ...
  /// Opaque JSON settings handed to the plugin at load time.
  pub settings: String,
}
  ...
  pub fn to_map(&self) -> Result<BTreeMap<String, String>> {
    ...
    map.insert(
      ...
      serde_json::to_string(&self.capabilities)
        .map_err(|err| PluginError::Manifest(format!("failed to encode capabilities: {err}")))?,
    );
    map.insert(
      HOOKS_KEY.to_string(),
      serde_json::to_string(&self.hooks)
        .map_err(|err| PluginError::Manifest(format!("failed to encode hooks: {err}")))?,
    );
    map.insert(
      SELECTOR_KEY.to_string(),
      serde_json::to_string(&self.selector)
        .map_err(|err| PluginError::Manifest(format!("failed to encode selector: {err}")))?,
    );
...
fn required<'a>(map: &'a BTreeMap<String, String>, key: &str) -> Result<&'a str> {
  map
    .get(key)
    .map(String::as_str)
    .ok_or_else(|| PluginError::Manifest(format!("missing required key: {key:?}")))
}

/// Decode a JSON-encoded manifest value.
fn parse_json<T: serde::de::DeserializeOwned>(raw: &str, key: &str) -> Result<T> {
  serde_json::from_str(raw)
    .map_err(|err| PluginError::Manifest(format!("invalid JSON in {key:?}: {err}")))
}
...
mod tests {
  ...
  fn rejects_unknown_capabilities() {
    ...
    let result = PluginManifest::from_map(&map);
    assert!(matches!(result, Err(PluginError::Manifest(err)) if err.contains("http")));
  }
  ...
  fn full_manifest_round_trips_semantically() {
    let manifest = PluginManifest::from_map(&full_map()).unwrap();
    let reparsed = PluginManifest::from_map(&manifest.to_map().unwrap()).unwrap();
