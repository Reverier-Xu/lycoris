...
- 用户如何审核、导入、下载并安装 skill。

协议、事件和磁盘布局细节，见 [lycoris-protocol-and-storage.md](lycoris-protocol-and-storage.md)。
用户主动添加的信息来源和 RAG 式检索，见 [lycoris-knowledge-base-and-retrieval.md](lycoris-knowledge-base-and-retrieval.md)。
...
- skill 可以由长期沉淀产生，也可以由用户手动导入或下载安装，但所有写入都必须可审计。
- skill tree 记录的是操作频率、触发条件和技能候选，不是另一个隐藏 prompt。
- 个人知识库不是长期记忆；知识库保存用户主动添加的资料 source，长期记忆保存系统提炼出的稳定结论。
...
Lycoris 的 skill 有三类来源：

- 长期沉淀：由长期记忆和历史操作频率产生 skill candidate。
- 手动导入：用户从本地路径导入已有 skill。
- 下载安装：用户从远程 registry、git repository、URL 或包源下载 skill。

skill tree 只负责第一类来源的发现和建议。手动导入和下载安装不需要先成为 skill candidate，但仍然必须进入审核、校验和审计流程。
...
1. 获取 skill 到本地 staging 区。
2. 解析 manifest、入口文件和权限声明。
3. 检查文件结构、名称冲突、版本和 scope。
4. 展示来源、差异、权限需求和触发条件。
5. 用户确认后写入 global 或 project skill 目录。
6. 写入 `skill.imported`、`skill.downloaded` 或 `skill.installed` 事件。

远程下载不能静默启用 skill。下载完成只代表进入本地 staging，安装仍需要用户确认。
...
- `origin = generated`：由 skill tree candidate 生成；
- `origin = local_import`：用户从本地导入；
- `origin = remote_download`：用户从远程下载；
- `source_ref`：本地路径、URL、registry id 或 git revision；
- `approved_by`：执行安装确认的 actor。
