...
3. 判断是否允许检索 global 或 project personal knowledge。
4. 按当前 prompt、path cluster、工具类型和最近任务筛选相关条目。
5. 在预算内注入运行时上下文。
...
general session 和临时对话可以搜索个人知识库，但必须先判断 scope：

- global knowledge 可以被同一 actor 的 general session 检索。
- project knowledge 只能在 project root 匹配或用户显式选择项目时检索。
- 用户显式要求搜索知识库时，应优先执行检索。
- runtime 自主判断需要检索时，必须记录 `knowledge.search_requested` 事件。
- 检索结果必须包含 source、document、chunk 引用。
- 低置信度或无结果时，不应把知识库检索伪装成已有上下文。
...
- 导入或下载必须是显式命令。
- 下载完成只进入 staging，不自动启用。
- 安装前必须展示来源、权限需求、scope 和潜在冲突。
- 安装后必须记录 `skill.imported` 或 `skill.downloaded`，并记录 `skill.installed`。
...
- 手动导入和下载的 skill 是否需要显式确认。

### 12.7 个人知识库

- 临时会话是否能按需检索 global knowledge；
- project knowledge 是否只在 project root 匹配或显式选择时检索；
- knowledge search 是否记录可审计事件；
- 检索结果是否带 source、document、chunk 引用。
